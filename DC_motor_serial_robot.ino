...
const uint8_t IR_sensor_right = 1; // for incoming serial data
...
void parse_cmd_buffer() {
  ...
  else if (strncmp(cmd_buffer, "LeftMotor!", 10) == 0) {
    if (str2hex(cmd_buffer + 10, &val)) {
      LeftMotorSpeed = val;
    }
  } 
  else if (strcmp(cmd_buffer, "LeftMotor?") == 0) {
    Serial.print(LeftMotorSpeed, HEX);
    Serial.print("\r\n");
  }
 else if (strncmp(cmd_buffer, "RightMotor!", 11) == 0) {
    if (str2hex(cmd_buffer + 11, &val)) {
      RightMotorSpeed = val;
    }
  } 
 else if (strcmp(cmd_buffer, "RightMotor?") == 0) {
    Serial.print(RightMotorSpeed, HEX);
    Serial.print("\r\n");
  }
}
